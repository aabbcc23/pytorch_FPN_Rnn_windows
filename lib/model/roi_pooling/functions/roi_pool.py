...
from .._ext import roi_pooling_cpu
import pdb
...
class RoIPoolFunction(Function):
    ...
    def forward(ctx, features, rois): 
        ...
        if not features.is_cuda:
            ...
            roi_pooling_cpu.roi_pooling_forward_cuda(ctx.pooled_height, ctx.pooled_width, ctx.spatial_scale,
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.feature_size is not None and grad_output.is_cuda:
            batch_size, num_channels, data_height, data_width = ctx.feature_size
        ...
        elif ctx.feature_size is not None :
            batch_size, num_channels, data_height, data_width = ctx.feature_size
            grad_input = grad_output.new(batch_size, num_channels, data_height, data_width).zero_()

            roi_pooling_cpu.roi_pooling_backward(ctx.pooled_height, ctx.pooled_width, ctx.spatial_scale,
                                                  grad_output, ctx.rois, grad_input, ctx.argmax)
