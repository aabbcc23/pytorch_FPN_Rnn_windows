#include <TH/TH.h>
#include <math.h>
int roi_pooling_forward(int pooled_height, int pooled_width, float spatial_scale,THFloatTensor * features, THFloatTensor * rois, THFloatTensor * output)
{
...
int roi_pooling_backward(int pooled_height, int pooled_width, float spatial_scale,THFloatTensor * top_grad, THFloatTensor * rois, THFloatTensor * bottom_grad, THIntTensor * argmax)
{
    // Grab the input tensor
    ...
    int * argmax_flat = THFloatTensor_data( argmax);

    // Number of ROIs
    int num_rois = THIntTensor_size( rois, 0);
    int size_rois = THIntTensor_size( rois, 1);
    if (size_rois != 5)
    {
        return 0;
    }
    ...
    // batch size
    int batch_size = THIntTensor_size( bottom_grad, 0);
    // if (batch_size != 1)
    // {
    //     return 0;
    // }
    // data height
    int data_height = THIntTensor_size( bottom_grad, 2);
    // data width
    int data_width = THIntTensor_size( bottom_grad, 3);
    // Number of channels
    int num_channels = THIntTensor_size( bottom_grad, 1);
